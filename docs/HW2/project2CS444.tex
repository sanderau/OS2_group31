...
\begin{document}
    ...
    %\end{flushright}
       ...
       \section{Purpose of the Assignment}
            The purpose of the assignment, in our opinion, was to gain a better understanding of how most current linux block I/O schedulers work.
        ...
        \section{What We Learned}
            There was a lot to take from this assignment. We only barely scratched the surface of writing modules and block I/O schedulers as well. The big take aways from this assignmnet was more big picture stuff. 
            
            We learned the difference between between schedulering algorithms. LOOK is a scheduler that is very similar the SSTF scheduler except that it avoids starvation. It will travel back and forth across the disk organizing the list in a way where the scheduler will always hit the next closest sector on the disk minimizing the time it spend searching. CLOOK is similar to LOOK except for the fact it travels in only one direction, either out towards the disk or outside towards inside, this takes advantage of the fact the arm of the disk is much faster traversing large distances and avoids starvation of sectors on the edges due to the nature of the algorithm hitting the edges more often.
